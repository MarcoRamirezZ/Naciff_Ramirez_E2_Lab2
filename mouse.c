...
//Actions to be done with either the "mouse" or the "keyboard"
...
//This function is the one that sends data via USB
//First it restarts both keyboard and mouse buffers
//After that and depending on which part of the procedure it currently is
//it enters the next task and so on
static usb_status_t USB_DeviceHidMouseAction(void)
{
	static uint8_t delay_counter = FALSE;
	static uint8_t k_or_m = FALSE; //False means it is the keyboard

	/*Refreshes the mouse and keyboard buffers on the specific bytes used*/
	g_UsbDeviceHidMouse.K_buffer[0] = 0x02U; //Keyboard ID
	g_UsbDeviceHidMouse.K_buffer[3] = 0x00U; // to 0x00 because it already had something before, reseting
	g_UsbDeviceHidMouse.K_buffer[4] = 0x00U; //This are Keyboard first and second byte buffer
	g_UsbDeviceHidMouse.M_buffer[0] = 0x01U; //Mouse ID
	g_UsbDeviceHidMouse.M_buffer[1] = 0x00U; //Mouse button
	g_UsbDeviceHidMouse.M_buffer[2] = 0x00U; //X coordinate
	g_UsbDeviceHidMouse.M_buffer[3] = 0x00U; //Y coordinate

	if(INCOMPLETE== openPaint_flag && KEYBOARD_DELAY<delay_counter)
	{
		openPaint_flag = MSPaint(g_UsbDeviceHidMouse.K_buffer);
		delay_counter = FALSE;
		k_or_m = FALSE;
	}
	if(INCOMPLETE == drawFigure_flag && KEYBOARD_DELAY<delay_counter && COMPLETE==openPaint_flag)
	{
		drawFigure_flag = Square(g_UsbDeviceHidMouse.M_buffer);
		delay_counter = FALSE;
		k_or_m = TRUE;
	}
	if(INCOMPLETE == openNotepad_flag && KEYBOARD_DELAY<delay_counter && COMPLETE==drawFigure_flag)
	{
		openNotepad_flag = Notepad(g_UsbDeviceHidMouse.K_buffer);
		delay_counter = FALSE;
		k_or_m = FALSE;
	}
	if(INCOMPLETE == moveLeft_flag && MOUSE_DELAY<delay_counter && COMPLETE==openNotepad_flag)
	{
		moveLeft_flag = moveLeft(g_UsbDeviceHidMouse.M_buffer);
		delay_counter = FALSE;
		k_or_m = TRUE;
	}
	if(INCOMPLETE == writeText_flag && KEYBOARD_DELAY<delay_counter && COMPLETE ==moveLeft_flag )
	{
		writeText_flag = write_notepad(g_UsbDeviceHidMouse.K_buffer);
		delay_counter = FALSE;
		k_or_m = FALSE;
	}
	if(INCOMPLETE == moveRight_flag && MOUSE_DELAY<delay_counter && COMPLETE ==writeText_flag )
	{
		moveRight_flag = moveRight(g_UsbDeviceHidMouse.M_buffer);
		delay_counter = FALSE;
		k_or_m = TRUE;
	}
	if(INCOMPLETE == pasteText_flag && KEYBOARD_DELAY<delay_counter && COMPLETE ==moveRight_flag )
	{
		pasteText_flag = paste_notepad(g_UsbDeviceHidMouse.K_buffer);
		delay_counter = FALSE;
		k_or_m = FALSE;
	}
	...
	if(!k_or_m)
	{//For keyboard
	return USB_DeviceHidSend(g_UsbDeviceHidMouse.hidKeyboardHandle, USB_HID_MOUSE_ENDPOINT_IN, g_UsbDeviceHidMouse.K_buffer,
		                             USB_HID_KEYBOARD_REPORT_LENGTH);
	}
	else
	{//For mouse
	return USB_DeviceHidSend(g_UsbDeviceHidMouse.hidMouseHandle, USB_HID_MOUSE_ENDPOINT_IN, g_UsbDeviceHidMouse.M_buffer,
...
static void USB_DeviceApplicationInit(void) //modified
{
    USB_DeviceClockInit();
#if (defined(FSL_FEATURE_SOC_SYSMPU_COUNT) && (FSL_FEATURE_SOC_SYSMPU_COUNT > 0U))
    ...
    g_UsbDeviceHidMouse.attach = 0U;
    g_UsbDeviceHidMouse.hidMouseHandle = (class_handle_t)NULL;//added
    g_UsbDeviceHidMouse.hidKeyboardHandle = (class_handle_t)NULL;//added
    g_UsbDeviceHidMouse.deviceHandle = NULL;
    g_UsbDeviceHidMouse.M_buffer = s_MouseBuffer; //addedd
    g_UsbDeviceHidMouse.K_buffer = s_KeyboardBuffer;//added
    ...
    ((defined(FSL_FEATURE_SOC_USBDCD_COUNT) && (FSL_FEATURE_SOC_USBDCD_COUNT > 0U)) ||        \
     (defined(FSL_FEATURE_SOC_USBHSDCD_COUNT) && (FSL_FEATURE_SOC_USBHSDCD_COUNT > 0U)))
        ...
        g_UsbDeviceHidMouse.hidMouseHandle = g_UsbDeviceHidConfigList.config->classHandle;//added
        g_UsbDeviceHidMouse.hidKeyboardHandle = g_UsbDeviceHidConfigList.config->classHandle;//added
    }
